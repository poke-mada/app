                  ...
                  <v-tooltip location="top">
                    <template #activator="{ props }">
                      <template v-if="stat === 'hp'">
                        <v-progress-linear class="paddinBars" v-bind="props" :model-value="value"
                          :max="pokemon.battle_data.stats.max_hp" height="18"
                          :color="stat === 'attack' ? '#0600FF' : '#D5048D'" rounded />
                      </template>
                      <template v-else>
...
export default {
  ...
  computed: {
    ...
    baseStats() {
      ...
      // Detectar correctamente el HP según el tipo de equipo
      const hp = this.team === 'you'
        ? this.pokemon.battle_data?.current_hp ?? 0
        : this.pokemon.current_hp ?? 0;

      // Si es mi equipo y tengo datos completos
      ...
      // Para enemigos, obtener los base_stats desde el archivo
