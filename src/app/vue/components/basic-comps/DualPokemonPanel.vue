      ...
      <v-row class="mt-0">
        <v-col cols="2">
          ...
          <v-row>
            <v-col>
              <div v-if="pokemon">
                <v-img v-for="(type, i) in pokemon_types" :key="i" :src="`./assets/types/${type_name(type.name)}.png`"
                  width="32" inline />
              </div>
            </v-col>
          </v-row>
        ...
        <v-col cols="2">
          ...
          <p v-if="pokemon_types" class="text-center">
            {{pokemon_types.map((v) => v.name).join("/")}}
          </p>
